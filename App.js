...
//    const { data: recommendations } = await axios.post(config.API_URL, {"userId": this.state.selectedUser}).then(response => {
//      const res = response;
//      console.log(res);
//      }).catch(err => {
//          const error = err;
...
//    const recommendations = [34, 32, 893, 1];
    ...
    console.log('USER ID : ' + this.state.selectedUser);
    ...
    if (this.state.step === 1) {
      return (
        <View style={styles.container}>
          ...
          <FlatList
            style={{ maxHeight: 200 }}
            data={this.state.recommendations.map(key => ({key: key.toString()}))}
